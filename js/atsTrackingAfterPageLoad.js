$(function(){
	...
	if(/index\.html/.test(window.location.pathname)){
		...
		*/
		// EX : view promotion GA4
		dataLayer.push({ ecommerce: null });  // Clear the previous ecommerce object.

function gtm_GA4_promo() {
    var allPromotions = []; // Crée un tableau pour stocker toutes les promotions

    // Itère sur chaque promotion et ajoute-la au tableau allPromotions
    for (var i = 0; i < promotions.length; i++) {
        allPromotions.push({

            item_name:promotions[i].item_name,
			item_id:promotions[i].item_id,
            creative_name: promotions[i].creative_name,
            creative_slot: promotions[i].creative_slot,
            promotion_id: promotions[i].promotion_id,
            promotion_name: promotions[i].promotion_name
                }
            
        );
    }

    // Envoie toutes les promotions en un seul événement
...
dataLayer.push({
						...
						items : [
							{
								...
								creative_slot: promotions[v].creative_slot,
	...
	if(/destinations\.html/.test(window.location.pathname)){
		...
		*/
		...
		// EX : GA4 - GTM - view_item_list 


		function gtm_GA4_sendViewItemEvent() {
			var items = [];
		...
		$('a.btn-primary').on('click', function(e){
            ...
            */
			...
			dataLayer.push({
				...
				ecommerce: {
					currency: 'EUR',
					value: travelD.price*quantity,
					items: [
						{
					'item_id': travelD.id,
					'item_name': travelD.name,
					'item_category': travelD.category,
					'price': travelD.price,
					'quantity' : quantity
						}
					]
	...
	if(/basket\.html/.test(window.location.pathname)){
		...
		if(JSON.parse(getCookie('55Basket')) || cart55.length <1) {
			...
			*/
			...
			// EX : GTM - GA4 - view_cart 



			function gtm_ga4_view_cart() {
				var items = [];
				var totalValue = 0; // Initialiser la valeur totale à 0
				for (var i = 0; i < products.length; i++) {
					var itemTotal = products[i].price * products[i].quantity; // Calculer le total de cet article
					totalValue += itemTotal; // Ajouter le total de cet article à la valeur totale
					items.push({
						'item_id': products[i].item_id,
						'item_name': products[i].item_name,
						'item_category': products[i].item_category,
						'item_variant': products[i].item_variant,
						'price': products[i].price,
						'quantity': products[i].quantity // Utiliser la quantité associée à chaque produit
					});
				}
			...
			$('.remove').on('click', function(e){
                ...
                */
				...
				dataLayer.push({
  				...
  				ecommerce: {
    				currency: 'EUR',
    				value: productRemoved.price*productRemoved.quantity,
    				items: [
    						{
						'item_id': productRemoved.id,
						'item_name': productRemoved.name,
						'item_category': productRemoved.category,
						'price': productRemoved.price,
						'quantity' : productRemoved.quantity
					       }
					      ]
				         }
			      });
		...
		$('#confirmReservation').on('click', function(e){
			...
			if(name != '' && adress!='' && zip != '' && city != ''){
				...
				*/
				...
				// EX : GTM - GA4 - add_shipping_info

				function gtm_ga4_add_shipping_info() {
						var items = [];
						var totalValue = 0; // Initialiser la valeur totale à 0
						for (var i = 0; i < products.length; i++) {
							var itemTotal = products[i].price * products[i].quantity; // Calculer le total de cet article
							totalValue += itemTotal; // Ajouter le total d
							items.push({
							'item_id': products[i].item_id,
							'item_name': products[i].item_name,
							'item_category': products[i].item_category,
					...
					dataLayer.push({
  							...
  							ecommerce: {
							shipping_tier:'DHL',
							value: totalValue, // Utiliser la valeur totale calculée
    						currency: 'EUR',
							items
							}
						}
				...
				gtm_ga4_add_shipping_info(); // Appel de la fonction pour déclencher l'événement
				...
				document.getElementById('submitButton').addEventListener('click',function(){
					...
					*/
					...
					// EX : GTM - GA4 - add_payment_info 




					function gtm_GA4_add_payment() {
						var items = [];
						var totalValue = 0; // Initialiser la valeur totale à 0
						for (var i = 0; i < products.length; i++) {
							var itemTotal = products[i].price * products[i].quantity; // Calculer le total de cet article
					...
					gtm_GA4_add_payment(); // Appel de la fonction pour déclencher l'événement
		...
		$('#cancelOrder').on('click', function(e){
			...
			*/
			...
			dataLayer.push({
  			...
  			ecommerce: {
			'currency':'EUR',	
    		'transaction_id': cancelOrder.orderRef
			}});
