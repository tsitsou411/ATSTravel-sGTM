...
//EX : Add page_type -  dataLayer.push Page_type
...
var pagePathElements = pagePath.split('/');
var lastElement = pagePathElements[pagePathElements.length - 1];
var $page_type = customDimension[lastElement] || '';
...
})(window,document,'script','dataLayer','GTM-WN3VJPTW');
...
if(/checkout\.html/.test(window.location.pathname)){
	...
	if(JSON.parse(getCookie('55Basket')) || cart55.length <1){
		...
		*/
		...
		// EX : GTM - GA4 - begin_checkout 

		function gtm_GA4_beginCheckoutEvent() {
			var items = [];
...
if(/thankyou\.html/.test(window.location.pathname)){
	...
	*/
	...
	function gtm_GA4_purchaseEvent() {
		var items = [];
		for (var i = 0; i < products.length; i++) {
			items.push({
				'item_id': products[i].item_id,
...
gtm_GA4_purchaseEvent(); // Appel de la fonction pour déclencher l'événement
